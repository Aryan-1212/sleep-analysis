...
const Questions = () => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen py-20 px-4 sm:px-6 lg:px-8 relative">
        <StarryBackground />
        <div className="max-w-3xl mx-auto bg-white/10 backdrop-blur-md rounded-2xl shadow-xl p-8 transform transition-all duration-500 hover:scale-[1.02] border border-white/20">
          <div className="text-center">
            <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-purple-500 mx-auto mb-4"></div>
            <h2 className="text-2xl font-bold text-white mb-4">Analyzing Your Sleep Patterns...</h2>
            <p className="text-gray-300">Please wait while we process your responses and generate personalized recommendations.</p>
          </div>
        </div>
      </div>
    );
  ...
  if (showResults && predictionResult) {
    return (
      <div className="min-h-screen py-20 px-4 sm:px-6 lg:px-8 relative">
        <StarryBackground />
        <div className="max-w-4xl mx-auto bg-white/10 backdrop-blur-md rounded-2xl shadow-xl p-8 transform transition-all duration-500 hover:scale-[1.02] border border-white/20">
          ...
          <div className="bg-white/5 rounded-xl p-6 mb-8">
            <h3 className="text-xl font-semibold text-white mb-4">Your Responses Summary</h3>
            <div className="grid md:grid-cols-2 gap-4">
              {questions.map((q, index) => (
                <div key={index} className="bg-white/5 rounded-lg p-3">
                  <p className="text-white font-medium text-sm mb-1">{q.question}</p>
                  <p className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-400 text-sm">
                    {answers[index]}
                  </p>
                </div>
              ))}
          ...
          <div className="flex gap-4">
            <button
              ...
              className="flex-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white py-3 px-6 rounded-xl hover:from-purple-700 hover:to-pink-700 transform transition-all duration-300 hover:scale-105 hover:shadow-lg"
            >
              Retake Quiz
            </button>
            <button
              onClick={() => navigate('/dashboard')}
              className="flex-1 bg-gradient-to-r from-blue-600 to-cyan-600 text-white py-3 px-6 rounded-xl hover:from-blue-700 hover:to-cyan-700 transform transition-all duration-300 hover:scale-105 hover:shadow-lg"
            >
              Go to Dashboard
            </button>
          </div>
      ...
      <div className="max-w-3xl mx-auto bg-white/10 backdrop-blur-md rounded-2xl shadow-xl p-8 transform transition-all duration-500 hover:scale-[1.02] border border-white/20">
        <div className="mb-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-white bg-white/10 px-4 py-2 rounded-full">
        ...
        <div className="mb-8">
          <h3 className="text-2xl font-semibold text-white mb-6 transform transition-all duration-300 hover:scale-105">
